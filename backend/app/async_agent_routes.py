...
def active_run_ids() -> set[str]:
    return set(_active_tasks)


def _task_payload(goal: str, autonomy: int, plan: Any, created: str, task_id: str) -> dict[str, Any]:
    return {"id": task_id, "goal": goal, "autonomy": autonomy, "status": "planned", "created_at": created, "updated_at": created, "plan_version": plan.version, "steps": [step.model_dump() for step in plan.steps], "events": [], "verification": None, "evaluation": None, "human_review": None, "result": None}
...
async def _execute(run_id: str, task: dict[str, Any], run: Any) -> None:
    try:
        ...
        if current:
            current["status"] = "cancelled"; current["phase"] = "cancelled"; core.agent_runs[run_id] = current; core.store.save_agent_run(current)
        raise
    except Exception as exc:
        task["status"] = "failed"; task["result"] = f"Async agent execution failed: {exc}"; core.emit(task["id"], "agent_background_failure", error=str(exc)); core.persist(task)
        current = core.agent_runs.get(run_id)
        if current:
            current["status"] = "failed"; current["phase"] = "failed"; current["error"] = str(exc); core.agent_runs[run_id] = current; core.store.save_agent_run(current)
    finally:
...
async def run_agent_async(request: core.AgentRunCreate):
    ...
    core.tasks[task_id] = task; core.emit(task_id, "agent_task_created", goal=request.goal, autonomy=request.autonomy)
    run = new_run(task, core.now, request.max_iterations, request.max_retries, request.confidence_threshold)
    core.agent_runs[run.run_id] = run.as_dict(); core.store.save_agent_run(core.agent_runs[run.run_id]); core.persist(task)
    async def work() -> None: await _execute(run.run_id, task, run)
    try:
...
def async_status(run_id: str):
    ...
    return {"run_id": run_id, "active": run_id in _active_tasks, "status": run.get("status"), "run": run, "task": core.tasks.get(run["task_id"])}
