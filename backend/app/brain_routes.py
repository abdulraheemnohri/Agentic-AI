...
class LocalModelRegister(BaseModel):
    ...
    provider: str = Field(default="local-openai-compatible", min_length=1, max_length=80)

class System2Active(BaseModel):
    model_id: str = Field(min_length=1, max_length=100)

class System1Policy(BaseModel):
    mode: str = Field(pattern="^(any|all|consensus)$")
    minimum_confidence: float = Field(ge=0, le=1)
    minimum_reviews: int = Field(ge=1, le=20)
...
@router.get("/status")
def brain_status():
    return {"runtime": runtime.status(), "system1": list_system1_models(), "system2": list_models(), "registered_system2": list_registered("system2"), "system1_policy": get_system1_policy(), "system1_authoritative": True, "system2_local_only": True}

@router.get("/models")
def brain_models():
    return {"system1": list_system1_models(), "system2": list_models(), "registered_system2": list_registered("system2"), "active_system2": runtime.active_system2, "persistent_active_system2": get_active_system2(), "system1_policy": get_system1_policy()}
...
def select_model(request: ModelSelect):
    try:
        result = set_active_model(request.model_id); set_active_system2(request.model_id); return result
    except ValueError as exc: raise HTTPException(404, str(exc)) from exc

@router.post("/system2/local")
def register_local_model(request: LocalModelRegister):
    try:
        result = register_local_system2(request.model_id, request.base_url, request.model_name)
        register_system2(request.model_id, request.provider, request.base_url, request.model_name)
        return result | {"persisted": True}
    except ValueError as exc: raise HTTPException(422, str(exc)) from exc

@router.delete("/system2/local/{model_id}")
def delete_local_model(model_id: str):
    remove_system2(model_id); return {"removed": model_id}
...
def update_system1_policy(request: System1Policy):
    try:
        policy = set_system1_policy(request.mode, request.minimum_confidence, request.minimum_reviews)
        policy["fail_closed_on_disagreement"] = request.fail_closed_on_disagreement
        return policy
    except ValueError as exc: raise HTTPException(422, str(exc)) from exc
...
async def system1_council(request: System1ReviewRequest):
    ...
    policy = CouncilPolicy(stored.get("mode", "consensus"), float(stored.get("minimum_confidence", .5)), int(stored.get("minimum_reviews", 1)), bool(stored.get("fail_closed_on_disagreement", True)))
    decision = decide(review.get("reviews", []), policy)
    return {"reviews": review.get("reviews", []), "council": decision, "policy": policy.__dict__, "system1_authoritative": True}
