...
class ToolPolicyUpdate(BaseModel):
    policy: str | None = None; enabled: bool | None = None; risk: str | None = None
class HumanReview(BaseModel):
    score: float = Field(ge=0, le=1); label: str = "reviewed"; notes: str = ""
class GoldenCaseCreate(BaseModel):
    name: str = Field(min_length=1, max_length=200); goal: str = Field(min_length=1, max_length=10000)
    expected_checks: dict[str, bool] = Field(default_factory=dict); expected_status: str = "passed"; tags: list[str] = Field(default_factory=list)
class MemoryCreate(BaseModel):
    content: str = Field(min_length=1, max_length=20000); kind: str = "episodic"; importance: float = Field(default=0.5, ge=0, le=1); metadata: dict[str, Any] = Field(default_factory=dict)
class MemoryUpdate(BaseModel):
    content: str | None = Field(default=None, min_length=1, max_length=20000); kind: str | None = None; importance: float | None = Field(default=None, ge=0, le=1); metadata: dict[str, Any] | None = None

def run_evaluation(task: dict[str, Any]) -> dict[str, Any]:
    result = evaluate_task(task, review=task.get("human_review")); task["evaluation"] = result
...
def tool_detail(tool_name: str):
    ...
    if tool_name not in tools_map: raise HTTPException(404, "tool_not_found")
...
def update_tool_policy(tool_name: str, request: ToolPolicyUpdate):
    try: return set_policy(tool_name, request.policy, request.enabled, request.risk).as_dict()
    except KeyError as exc: raise HTTPException(404, "tool_not_found") from exc
    except ValueError as exc: raise HTTPException(422, str(exc)) from exc

@app.post("/api/tasks")
def create_task(request: TaskCreate):
    task_id, created, plan = str(uuid4()), now(), build_default_plan(request.goal)
    ...
    tasks[task_id] = task; emit(task_id, "task_created", goal=request.goal, autonomy=request.autonomy); return task
...
async def run_agent(request: AgentRunCreate):
    ...
    tasks[task_id] = task; emit(task_id, "agent_task_created", goal=request.goal, autonomy=request.autonomy)
    run = new_run(task, now, request.max_iterations, request.max_retries, request.confidence_threshold); agent_runs[run.run_id] = run.as_dict()
    def persist_run(current) -> None:
        agent_runs[current.run_id] = current.as_dict(); store.save_agent_run(agent_runs[current.run_id]); persist(task)
    final_run = await agent_kernel.run(task, run, persist_run)
    if task.get("evaluation"): store.save_evaluation(task_id, task["evaluation"], now())
    persist(task); agent_runs[final_run.run_id] = final_run.as_dict()
    return {"run": final_run.as_dict(), "task": task}

@app.get("/api/agent/runs")
def list_agent_runs(limit: int = Query(default=100, ge=1, le=500)): return {"count": len(store.load_agent_runs(limit)), "runs": store.load_agent_runs(limit)}
...
def get_agent_run(run_id: str):
    ...
    return {"run": run, "task": tasks.get(run["task_id"])}
@app.get("/api/agent/{run_id}/trace")
def agent_trace(run_id: str):
    run = agent_runs.get(run_id) or store.get_agent_run(run_id)
    if not run: raise HTTPException(404, "agent_run_not_found")
    return {"run_id": run_id, "stream": run.get("stream", []), "events": store.load_trace_events(run["task_id"]) or observer.list(run["task_id"])}
@app.post("/api/agent/{run_id}/cancel")
def cancel_agent(run_id: str):
    run = agent_runs.get(run_id) or store.get_agent_run(run_id)
    if not run: raise HTTPException(404, "agent_run_not_found")
    agent_kernel.cancel(run_id); run["status"] = "cancel_requested"; run["phase"] = "recovering"; agent_runs[run_id] = run; store.save_agent_run(run)
    task = tasks.get(run["task_id"])
    if task: executor.cancel(task["id"]); task["status"] = "cancelled"; persist(task)
    return run

@app.get("/api/tasks")
def list_tasks(): return sorted(tasks.values(), key=lambda t: t["created_at"], reverse=True)
@app.get("/api/history/tasks")
def task_history(limit: int = Query(default=100, ge=1, le=500)): return {"count": len(tasks), "tasks": sorted(tasks.values(), key=lambda t: t["updated_at"], reverse=True)[:limit]}
...
def get_plan(task_id: str):
    ...
    plan = Plan.model_validate({"version": task["plan_version"], "goal": task["goal"], "steps": task["steps"]}); return {"version": plan.version, "goal": plan.goal, "steps": plan.steps, "order": topological_order(plan)}
...
def update_plan(task_id: str, request: PlanUpdate):
    ...
    plan.version = task["plan_version"] + 1; task.update({"plan_version": plan.version, "goal": plan.goal, "steps": [s.model_dump() for s in plan.steps]}); emit(task_id, "plan_updated", version=plan.version); return task
@app.post("/api/tasks/{task_id}/dry-run")
def dry_run(task_id: str):
    task = tasks.get(task_id)
    if not task: raise HTTPException(404, "task_not_found")
    plan = Plan.model_validate({"version": task["plan_version"], "goal": task["goal"], "steps": task["steps"]}); permissions = [{"step_id": s.id, "tool": s.tool, "allowed": authorization(s.tool, task["autonomy"])[0], "reason": authorization(s.tool, task["autonomy"])[1]} for s in plan.steps]
...
def replan(task_id: str):
    ...
    plan = build_default_plan(task["goal"]); plan.version = task["plan_version"] + 1; task.update({"plan_version": plan.version, "steps": [s.model_dump() for s in plan.steps], "status": "planned"}); emit(task_id, "plan_rebuilt", version=plan.version); return task
...
async def approve_task(task_id: str, request: Approval):
    ...
    if not request.approved: task["status"] = "cancelled"; emit(task_id, "plan_rejected"); return task
    task["status"] = "running"; emit(task_id, "plan_approved", version=task["plan_version"]); plan = Plan.model_validate({"version": task["plan_version"], "goal": task["goal"], "steps": task["steps"]}); await executor.execute_plan(task, topological_order(plan)); task["verification"] = verify_task(task)
    if task["status"] == "completed" and task["verification"]["passed"]: task["result"] = "V2.1 execution completed and verified."; emit(task_id, "verification_passed", verification=task["verification"])
    else: task["status"] = "failed" if task["status"] == "completed" else task["status"]; task["result"] = "Execution did not satisfy verification."; emit(task_id, "verification_failed", verification=task["verification"])
...
def task_trace(task_id: str):
    if task_id not in tasks: raise HTTPException(404, "task_not_found")
    return {"task_id": task_id, "events": store.load_trace_events(task_id) or observer.list(task_id)}
...
async def retry_task(task_id: str):
    ...
    task["status"] = "running"; emit(task_id, "retry_started"); plan = Plan.model_validate({"version": task["plan_version"], "goal": task["goal"], "steps": task["steps"]}); await executor.execute_plan(task, topological_order(plan)); task["verification"] = verify_task(task)
...
def get_memory(memory_id: str):
    item = next((m for m in recall("", limit=500) if m["memory_id"] == memory_id), None)
    if not item: raise HTTPException(404, "memory_not_found")
    return item
@app.get("/api/storage/stats")
def storage_stats():
    with store.connect() as db: counts = {table: db.execute(f"SELECT COUNT(*) FROM {table}").fetchone()[0] for table in ("tasks", "evaluations", "traces", "golden_cases", "regression_runs", "memories", "agent_runs")}
